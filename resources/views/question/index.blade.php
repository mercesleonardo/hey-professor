<x-app-layout>
    ...
    <x-container>
            ...
            <x-table>
                ...
                @foreach($questions->where('draft', false) as $question)
                    <x-table.tr>
                        <x-table.td>{{ $question->question }}</x-table.td>
                        <x-table.td>
                            <x-form :action=" route('question.destroy', $question)" delete delete onsubmit="return confirm('Are you sure?')">
                                <button type="submit" class="hover:underline text-blue-500">
                                    Delete
                                </button>
                            </x-form>
        ...
        <div class="dark:text-gray-300 uppercase font-bold mb-1">Archive Questions</div>
        ...
        <div class="dark:text-gray-400 space-y-4">
            <x-table>
                ...
                <tbody>
                @foreach( $archivedQuestions->where('draft', false) as $archivedQuestion)
                    <x-table.tr>
                        <x-table.td>{{ $archivedQuestion->question }}</x-table.td>
                        <x-table.td>
